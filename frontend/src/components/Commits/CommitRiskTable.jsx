      ...
      <table className="min-w-full divide-y divide-gray-700">
        ...
        <tbody className="divide-y divide-gray-700">
          {commits.map((commit, index) => {
            ...
            return (
              <tr key={index} className="hover:bg-gray-700/30 transition duration-150">
                <td className="px-6 py-4 whitespace-nowrap text-sm font-mono text-cyan-400">{commit.id}</td>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-400 text-center">{commit.files}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-300">
                  <span className="text-green-400 mr-2">{added}</span>
                  <span className="text-red-400">{deleted}</span>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <span className={`px-3 py-1 text-xs font-semibold rounded-full border ${getRiskClass(commit.risk)}`}>
                    {commit.risk}
                  </span>
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{commit.time}</td>
              </tr>
            );
          })}
        </tbody>
